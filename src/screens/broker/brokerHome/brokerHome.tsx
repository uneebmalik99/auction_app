...
export default function BrokerHome() {
  ...
  const handleViewAllAuctions = () => {
    navigation.navigate(screenNames.brokerAuctionItems);
  };
  ...
  const handleDrawerItemPress = (screen: string) => {
    ...
    if (screen === 'home') {
      ...
      navigation.navigate(screenNames.brokerInvoices);
  ...
  useEffect(() => {
    ...
    const userRole = String(currentUser.role ?? 'broker');

    // Join broker room with viewAllVehicles
    socket.emit('join', {
    ...
    socket.on('connect', () => {
      console.log('[Broker] Socket connected:', socket.id);
      setSocketConnected(true);
      setError(null);
    });

    socket.on('disconnect', (reason) => {
      console.warn('[Broker] Socket disconnected:', reason);
    ...
    socket.on('initial_bids', (incomingBids: VehicleBid[]) => {
      console.log('[Broker] Received initial_bids:', incomingBids.length);
    ...
    socket.on('bid_update', (updatedBid: VehicleBid & { endTime?: string; fullData?: any }) => {
      console.log('[Broker] Received bid_update:', updatedBid.auctionId, updatedBid.currentBid);
      ...
      setBids((prevBids) => {
        ...
        if (!existingBid && !isRestartedAuction) {
          console.log(`[Broker] Adding new live vehicle ${updatedBid.auctionId} to dashboard`);
        }
        ...
        if (existingBid) {
          ...
          console.log('[Broker] ✅ Adding new/restarted vehicle to dashboard:', updatedBid.auctionId);
    ...
    socket.on('auction_completed', (data: any) => {
      console.log('[Broker] Received auction_completed:', data);
      if (data && data.auctionId && data.status === 'pending') {
        ...
        console.log(`[Broker] Updated vehicle ${data.auctionId} status to 'pending'`);
        fetchVehicleStats();
      }
    });
  ...
  const handleViewBidDetails = (vehicleId: string) => {
    try {
      ...
      if (vehicle?.buyNowEnabled) {
        console.log('[Broker] Buy Now vehicle - bid details not available');
        return;
      }
      ...
      console.error('[Broker] Error in handleViewBidDetails:', error);
      setError('Failed to load bid details');
      setBidDetailsLoading(false);
    }
  };
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      {/* <Header title="Broker" showBackButton={false} /> */}
      <ScrollView
