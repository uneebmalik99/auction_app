...
import { formatRemainingTime, hasTimeRemaining } from '../../utils/methods';
...
export default function ItemDetails() {
  ...
  const [bidAmount, setBidAmount] = useState(itemParams?.startingPrice ?? 0);
  const [now, setNow] = useState(Date.now());
  const step = itemParams?.minimumBidIncrement ?? 0;
  const currentUser = useAppSelector(state => state.profile.user);
  const [item, setItem] = useState<AuctionItem | null>(itemParams ?? null);
  ...
  const { currentBid, bids, placeBid } = useAuctionBids({
    auctionId: itemParams?._id || (itemParams ?? null),
    initialBid: itemParams?.startingPrice ?? itemParams?.currentBid ?? 0, // ← Use actual auction starting/current bid
  });
  ...
  const loadSeller = async () => {
    ...
    try {
      ...
      const userData = await fetchUserById(itemParams?.sellerId as string);
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      ...
      >
            ...
            {item?.photos[0] ? (
              ...
              >
                {item.photos.map((uri, index) => (
                  <TouchableOpacity
                    onPress={() => openImagePreview(uri as string)}
                  >
                    <Image
        ...
        {/* <UserCard
          ...
          onPress={() => {
            navigation.navigate(screenNames.userDetails, {
              ...
              userId: item?.sellerId as string,
            });
          }}
        /> */}
        ...
        hasTimeRemaining(item?.biddingEndsAt) ? (
          <BidControls
            step={step}
          ...
          <View>
            {loadingSeller ? (
              ...
              <UserCard
                ...
                onPress={() => {
                  navigation.navigate(screenNames.userDetails, {
                    user: seller, // pass full user for instant load
                    userId: seller.id as string,
                  });
                }}
              />
            ) : (
              <View style={styles.emptySeller}>
