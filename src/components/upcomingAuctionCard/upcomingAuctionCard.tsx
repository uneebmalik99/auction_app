...
export default function UpcomingAuctionCard({ item, onPress }: UpcomingAuctionCardProps) {
  ...
  useEffect(() => {
    if (!item.biddingStartsAt) return;
  ...
  }, [item.biddingStartsAt]);

  const getTimeUntilStart = () => {
    if (!item.biddingStartsAt) return '00:00:00';
    const startTime = Date.parse(item.biddingStartsAt as string);
    ...
    >
      ...
      <View style={styles.imageContainer}>
        ...
        <View style={styles.upcomingBadge}>
          <Megaphone size={12} color={appColors.white} />
